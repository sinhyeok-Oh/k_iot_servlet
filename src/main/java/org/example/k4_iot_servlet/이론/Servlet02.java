...
/*
    1) 클리이언트가 요청을 보냄
        ...
        - 요청은 HTTP 프로토콜 사용 + GET/POST/PUT/DELETE 등의 메서드와 함계 전송

    2) 웹 서버가 ㅇ청을 받음
        - 톰켓(Tomcat)과 같은 웹 서버(서블릿 컨테이너 포함)가 8080 포트에서 대기 중인 상태
            >> 요청을 수신
    3) 서블릿 컨테이너가 요청을 처리
    ...
    cf) 리스너(Listener)
        - 서블릿 컨테이너가 동작하면서 특정 이벤트 발생시 실행되는 객체

    cf) 필터(Filter)
        - 전처리/후처리
        - 공통 인증 처리, 로김, 인코딩 설정, 요청 데이터 가공 등 (체인 방식 - 여러 개 연결 가능)
